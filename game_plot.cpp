...
void plot4_ending(Role *player_addr, Role *boss_addr, int player_loss){  //Plot for set4 ending by inputing pointer to player and boss struct and player_loss 
    ...
    cout << "blaming to the world why am I that unlucky." << endl; sleep_for(::seconds(sleep_t));
    cout << "That's life I guess" << endl; sleep_for(::seconds(sleep_t));
    cout << "Although we cannot control what we will run into today," << endl; sleep_for(::seconds(sleep_t));
    cout << "We are still able to control how we react to them." << endl; sleep_for(::seconds(sleep_t));
    cout << "STOP WASTING YOUR LIFE AND ENERGY ON SOMETHING OR SOMEONE MAKES YOU UNHAPPY!" << endl; sleep_for(::seconds(sleep_t));
    cout << "FOCUS ON YOURSELF MORE!" << endl; sleep_for(::seconds(sleep_t));
    cout << "Having a bad day today?" << endl; sleep_for(::seconds(sleep_t));
    cout << "Wanna beat up your annoying colleague and arrogant boss?" << endl; sleep_for(::seconds(sleep_t));
    cout << "Wanna have a break from this busy Hong Kong and life?" << endl; sleep_for(::seconds(sleep_t));
